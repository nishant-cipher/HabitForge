...
import { Users, Plus, X, Lock, Globe, Copy, CheckCircle2, LogOut, Search } from "lucide-react"
...
export function Clubs() {
    ...
    // Search: merge public clubs + user's private clubs, deduplicate
    const searchResults: Club[] = searchQuery.trim()
        ? (() => {
            const q = searchQuery.toLowerCase()
            const publicFiltered = publicClubs.filter((c: Club) => c.name.toLowerCase().includes(q) || c.description?.toLowerCase().includes(q))
            const privateJoined = myClubs.filter((c: Club) => c.isPublic === false && (c.name.toLowerCase().includes(q) || c.description?.toLowerCase().includes(q)))
            // Merge and deduplicate
            const seen = new Set<string>()
            return [...publicFiltered, ...privateJoined].filter(c => { if (seen.has(c._id)) return false; seen.add(c._id); return true })
        })()
        : []
        ...
        return (
            <div key={club._id} className="surface-card p-5 flex flex-col gap-3"
                ...
                <div className="flex items-start justify-between">
                    ...
                    <div className="h-4 w-4 flex-shrink-0 ml-2 opacity-40" style={{ fontSize: "14px" }}>✦</div>
                </div>
                ...
                {isOwner ? (
                    ...
                    <button
                        onClick={(e) => { e.stopPropagation(); leaveMutation.mutate(club._id) }}
                    ...
                    <button
                        onClick={(e) => { e.stopPropagation(); handleJoinPublic(club) }}
                    ...
                    <button
                        onClick={(e) => { e.stopPropagation(); handleJoinPrivate(club) }}
                        className="w-full py-2 rounded-lg text-xs font-semibold flex items-center justify-center gap-1 transition-all"
                ...
                <div className="flex flex-col gap-3">
                    ...
                    {searchResults.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                            {searchResults.map((club: Club) => renderClubCard(club))}
                        </div>
                    )}
                </div>
            )}

            {/* Normal layout (when no search query) */}
            {!searchQuery.trim() && (
                <>
                    {/* My Clubs Section */}
                    {myClubs.length > 0 && (
